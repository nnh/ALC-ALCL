...
#    2020.04.15  Agata.K  不一致リストも別で出力する
#*******************************************　 
...
# 2020.04.15 不一致リストも必要なため,Ptoshデータは全て残す（all.x=T）
...
result <- match[,c(36,1:35,37:59)]
result_not <- not.match[,c(36,1:35,37:59)]

# CSVファイル出力
write.csv(result, outfname, row.names=F, na="")
write.csv(result_not, nmatch_outfname, row.names=F, na="")
